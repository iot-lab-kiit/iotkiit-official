  ...
  return (
    <>
      ...
      {autoPromptShown && !isOpen && (
        <div className="fixed bottom-6 left-6 z-40 flex items-center gap-3 rounded-full border border-cyan-400/40 bg-[#070F2B]/90 px-4 py-2.5 backdrop-blur-2xl shadow-glow-cyan transition-all duration-300">
          <div className="flex h-3 w-3 items-center justify-center">
            <span className="h-2 w-2 rounded-full bg-cyan-400" />
          </div>
          <span className="font-mono text-xs text-cyan-200">
            Press <kbd className="rounded border border-cyan-400/40 bg-black/40 px-1.5 py-0.5 text-[10px] text-cyan-300">T</kbd> to launch Lab Terminal
          </span>
          <button
            onClick={() => setIsOpen(true)}
            className="rounded-full bg-cyan-500/20 p-1 hover:bg-cyan-500/40 text-cyan-300 transition-colors"
          >
      ...
      {!isOpen && (
        <button
          onClick={() => setIsOpen(true)}
          className="fixed bottom-6 right-20 z-40 flex h-12 w-12 items-center justify-center rounded-full border border-cyan-400/30 bg-[#070F2B]/80 text-cyan-400 backdrop-blur-xl shadow-glass transition-all hover:scale-110 hover:border-cyan-400 hover:shadow-glow-cyan"
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-md">
          <div
            className={`relative w-full max-w-2xl overflow-hidden rounded-3xl border border-cyan-500/30 bg-[#030712]/95 text-green-400 shadow-2xl transition-all duration-300 ${
              minimized ? "h-14" : "h-[480px]"
            }`}
          >
            {/* Terminal Window Header Bar */}
            <div className="flex items-center justify-between border-b border-white/10 bg-white/[0.04] px-6 py-3 font-mono text-xs text-cyan-300">
