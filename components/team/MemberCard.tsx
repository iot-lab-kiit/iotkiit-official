...
// Compact, text-only card for the full member grid. Member profile photos aren't
// available (private form uploads), so every card gets identical clean styling:
// name, an optional domain chip, and social links. No placeholder image, so a
// missing photo never reads as broken.
const MemberCard = ({ person }: { person: Person }) => {
  return (
    <div className="group flex h-full flex-col justify-between rounded-xl border border-gray-100 bg-white p-4 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-primary-200 hover:shadow-lg">
