...
export default function CustomCursor() {
  ...
  return (
    <>
      <div
        ref={cursorRef}
        className={`fixed top-0 left-0 w-2.5 h-2.5 bg-cyan-400 rounded-full pointer-events-none z-[99999] -translate-x-1/2 -translate-y-1/2 transition-opacity duration-300 hidden md:block ${
          hidden ? "opacity-0" : "opacity-100"
        }`}
      />
      <div
        ref={followerRef}
        className={`fixed top-0 left-0 w-8 h-8 border border-cyan-400/40 rounded-full pointer-events-none z-[99998] -translate-x-1/2 -translate-y-1/2 transition-opacity duration-300 hidden md:block ${
          hidden ? "opacity-0" : "opacity-100"
        }`}
      />
    </>
